			...
			<div id="nav">
				...
				<ul class="menu">
					<li><a id="logo" href="<%=request.getContextPath()%>"> <img
						src="<%=request.getContextPath()%>/imgs/logo.png"  alt="Logo HPRWEB" width="85px">
					</a></li>
					...
					<li><a href="<%=request.getContextPath() + ViewPaths.CARRITO%>"><fmt:message
								key="carrito" bundle="${traducciones}" /></a></li>
					<li>
						<form action="<%=ControllerPaths.CONTENIDO%>" method="post">
							<input type="hidden" name="<%=ParameterNames.ACTION%>"
								value="<%=Actions.BUSCAR%>" /> <input id="buscador" type="text"
								name="<%=ParameterNames.TITULO%>"
							...
							<div id="menudes" class="menudes-contido">
								<%-- <label><fmt:message key="categoria"
									...
									%>
								 --%></select> <label><fmt:message key="restriccionedad"
										bundle="${traducciones}" /></label> <select
								...
								</select> <label><fmt:message key="tipocontenido"
										bundle="${traducciones}" /></label> <select
